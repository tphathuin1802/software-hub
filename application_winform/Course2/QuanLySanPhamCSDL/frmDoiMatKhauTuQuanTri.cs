...
{
    public partial class frmDoiMatKhauTuQuanTri : Form
    {
        public int idnhanvien = -1;
        public frmDoiMatKhauTuQuanTri()
        {
            InitializeComponent();
        }
        ...
        private void buttonDoiMatKhau_Click(object sender, EventArgs e)
        {
            ...
            NhanVien loginNv = nvc.XemChiTiet(idnhanvien);
            if (loginNv != null)
            {
                ...
                loginNv.Password=textBoxPasswordMoi.Text;
                //tiến hành thay đổi mật khẩu thực sự trong CSDL:
                bool ret=nvc.DoiMatKhau(loginNv);
                if (ret)
                {
